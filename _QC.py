...
def qcMetrics(adata:sc.AnnData,
    *,
    ...
    if hemoglobin:
        _adata.var.loc[:,'Hb'] = _adata.var_names.isin(("HBA1", "HBA2", "HBB", "HBD", "HBE1", "HBG1", "HBG2", "HBM", "HBQ1", "HBZ"))
    if mitochondrion:
        _adata.var.loc[:,'Mito'] = bioquest.st.detects(string=_adata.var_names,pattern=r"^mt-",flags=re.I)
    if ribosome:
    ...
    # /* scatter plot */
    if qc_vars:
        iters = list(itertools.product(ks[[0,1]],keys[[False,False,mitochondrion,ribosome,hemoglobin]]))
        _, axes = plt.subplots(1,_n, figsize=(3*_n, 3))
        for i,(x,y) in enumerate(iters):
            sc.pl.scatter(_adata, x=x, y=y, color=batch_key,ax=axes[i],legend_loc="none",show=False)
        sc.pl.scatter(_adata, x=ks[0], y=ks[1], color=batch_key,ax=axes[-1],legend_loc="none",show=False)
        plt.subplots_adjust(wspace = 0.5)
        _export("QC_Scatter")
    else:
        sc.pl.scatter(_adata, x=ks[0], y=ks[1], color=batch_key,legend_loc="none",show=False)
        _export("QC_Scatter")
